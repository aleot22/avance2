...
SELECT nombre, apellido1, apellido2 FROM persona WHERE tipo = 'alumno' AND (telefono IS NULL OR telefono = '') ORDER BY apellido1, apellido2, nombre;

-- 3
SELECT nombre, apellido1, apellido2, fecha_nacimiento FROM persona WHERE tipo = 'alumno' AND YEAR(fecha_nacimiento) = 1999 ORDER BY apellido1, apellido2, nombre;

-- 4
SELECT nombre, apellido1, apellido2, nif FROM persona WHERE tipo = 'profesor' AND telefono IS NULL AND RIGHT(nif, 1) = 'K' ORDER BY apellido1, apellido2, nombre;

-- 5

SELECT nombre FROM asignatura WHERE curso = 3 AND id_grado = 7 AND cuatrimestre = 1;
...
SELECT aa.nombre as nombre_asignatura , cr.anyo_inicio, cr.anyo_fin FROM alumno_se_matricula_asignatura a INNER JOIN asignatura aa ON a.id_asignatura = aa.id INNER JOIN curso_escolar cr ON a.id_curso_escolar = cr.id INNER JOIN persona pa ON a.id_alumno = pa.id  WHERE pa.nif = '26902806M';

-- 8

SELECT DISTINCT d.nombre as nombre_departamento, a.nombre as nombre_signatura, a.id_grado as nombre_grado,  p.id_profesor FROM departamento d INNER JOIN profesor p ON d.id = p.id_departamento iNNER JOIN asignatura a ON p.id_profesor = a.id_profesor INNER JOIN grado g ON a.id_grado = g.id;


-- 1


SELECT d.nombre AS nombre_departamento, persona.apellido1 AS ape1, persona.apellido2 AS ape2, persona.nombre AS nombre_profesor FROM profesor p LEFT JOIN departamento d ON p.id_departamento = d.id JOIN persona ON p.id_profesor = persona.id  ORDER BY d.nombre, persona.apellido1, persona.apellido2, persona.nombre;

-- 2

SELECT persona.apellido1 as ape1, persona.apellido2 as ape2, persona.nombre AS nombre_profesor FROM profesor p LEFT JOIN departamento d ON p.id_departamento = d.id JOIN persona ON p.id_profesor = persona.id  WHERE d.id IS NULL ORDER BY persona.apellido1 , persona.apellido2, persona.nombre;

-- 3

SELECT departamento.nombre as nombre_apartamento FROM departamento  LEFT JOIN profesor  ON departamento.id = profesor.id_departamento WHERE profesor.id_profesor IS NULL ORDER BY nombre_apartamento;

-- 4

SELECT persona.apellido1 as ape1, persona.apellido2 as ape2, persona.nombre AS nombre_profesor FROM profesor p LEFT JOIN asignatura a ON p.id_profesor = a.id_profesor JOIN persona ON p.id_profesor = persona.id  WHERE a.id_profesor IS NULL ORDER BY persona.apellido1, persona.apellido2, persona.nombre;

-- 5

SELECT a.nombre as nombre_asignatura FROM asignatura a LEFT JOIN profesor p ON a.id_profesor = p.id_profesor WHERE p.id_profesor IS NULL ORDER BY a.nombre;

-- 6

SELECT d.nombre as nombre_departamento FROM departamento d LEFT JOIN profesor p ON d.id = p.id_departamento LEFT JOIN asignatura a ON p.id_profesor = a.id_profesor WHERE a.id IS NULL ORDER BY d.nombre;
...
-- 3
SELECT  d.nombre AS Nombre_departamento, COUNT(*) AS Numero_profesores FROM departamento d JOIN profesor ON d.id = profesor.id_departamento GROUP BY d.nombre ORDER BY COUNT(*)DESC; 

-- 4

SELECT d.nombre AS Nombre_departamento, COUNT(*) AS Numero_profesores FROM departamento d LEFT JOIN profesor p ON d.id = p.id_departamento GROUP BY d.nombre ;

-- 6 
SELECT  g.nombre AS Nombre_grado, COUNT(*) AS Numero_asignatura FROM grado g JOIN asignatura a ON g.id = a.id_grado GROUP BY g.nombre  HAVING COUNT(*) > 40;

-- 7
SELECT g.nombre AS Nombre_grado, a.tipo AS Tipo_asignatura, SUM(a.creditos) AS Suma_creditos FROM grado g JOIN asignatura a ON g.id = a.id_grado GROUP BY g.nombre, a.tipo order by SUM(a.creditos)Desc;
-- 10
